...
export const execute: CommandExecute = async (interaction: CommandInteraction): Promise<void> => {
  switch (interaction.options.getSubcommand()) {
    ...
    break;
    case 'instance': await instance(interaction, interaction.options.getString('type', true), interaction.options.getNumber('value', true));
    break;
    case 'set': await set(interaction, interaction.options.getString('category') as errorTypes, interaction.options.getString('type', true), interaction.options.getNumber('value', true));
    break;
    case 'call': await call(interaction, interaction.options.getString('method', true), interaction.options.getBoolean('value'));
    break;
    //no default
  }
};
...
async function call(interaction: CommandInteraction, type: string, value: boolean | null) {
  ...
  const callEmbed = new BetterEmbed({ color: Constants.color.normal, footer: interaction })
    .setTitle('Executed!')
