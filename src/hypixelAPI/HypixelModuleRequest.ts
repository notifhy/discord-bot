...
export class HypixelModuleRequest extends HypixelRequest {
    ...
    generateURLS(user: UserAPIData) {
        ...
        if (Number(user.lastLogin) > Number(user.lastLogout)) {
    ...
    async executeRequest(
        user: UserAPIData,
        urls: string[],
    ): Promise<[CleanHypixelPlayer, CleanHypixelStatus | undefined]> {
    ...
    static cleanPlayerData(
        rawHypixelPlayer: RawHypixelPlayer,
        userAPIData: UserAPIData,
    ) {
        const rawHypixelPlayerData: RawHypixelPlayer['player'] =
            rawHypixelPlayer.player;
        return {
            ...
            language:
                rawHypixelPlayerData.userLanguage ??
                userAPIData.language ??
                'ENGLISH',
